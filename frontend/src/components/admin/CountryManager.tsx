    ...
    <div className="p-6 rounded-2xl glass-panel border border-white/10 space-y-6">
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 mb-1">State Name</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 mb-1">Access Password</label>
            <input
              type="password"
              value={password}
          ...
          <div>
            <label className="block text-[11px] font-semibold text-slate-400 mb-1">Initial Treasury ($)</label>
            <input
              type="number"
              value={money}
        ...
        <div className="flex justify-end pt-2 border-t border-white/5">
          <button
            type="submit"
            disabled={isCreating}
            className="px-4 py-2 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-titan-950 font-black text-xs uppercase tracking-wider shadow-glow-cyan transition-all flex items-center gap-1.5 disabled:opacity-40"
          >
            {isCreating ? (
              <div className="w-3 h-3 border-2 border-titan-950 border-t-transparent rounded-full animate-spin" />
            ) : (
              <Plus className="w-4 h-4 text-titan-950" />
            )}
          ...
          <tbody className="divide-y divide-white/5 text-xs">
            {countries.map((c) => (
              <tr key={c.id} className="hover:bg-white/5 transition-colors">
                ...
                <td className="py-3 px-3 font-mono text-slate-300">{c.username}</td>
