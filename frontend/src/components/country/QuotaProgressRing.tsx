...
export const QuotaProgressRing: React.FC<QuotaProgressRingProps> = ({
  ...
  return (
    <div
      className={`p-6 rounded-2xl border transition-all shadow-soft-card ${
        isAllComplete
          ? 'bg-gradient-to-r from-amber-500/10 via-yellow-500/5 to-white dark:to-titan-900 border-amber-400/50 shadow-glow-gold/20'
          : 'bg-white dark:bg-titan-900 border-slate-200/80 dark:border-white/10'
      }`}
    >
      <div className="flex items-center justify-between flex-wrap gap-4 mb-4">
        <div>
          <span className="text-xs font-mono font-bold tracking-widest text-slate-500 dark:text-slate-400 uppercase flex items-center gap-1.5">
            <Target className="w-3.5 h-3.5 text-sky-500" />
            National Strategic Progress
          </span>
          <h3 className="font-display font-bold text-xl text-slate-950 dark:text-white mt-0.5 flex items-center gap-2">
            Import Quota Fulfillment
            {isAllComplete && (
              <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-amber-100 dark:bg-amber-400/20 border border-amber-300 dark:border-amber-400/50 text-amber-800 dark:text-amber-300 text-xs font-mono font-bold uppercase tracking-wider animate-pulse">
      ...
      <div className="flex flex-col md:flex-row items-center gap-6">
        ...
        <div className="relative flex items-center justify-center shrink-0">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center">
            <span className="text-2xl font-display font-black text-slate-950 dark:text-white leading-none">
              {unitPercentage}%
            </span>
            <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-0.5">
              Fulfilled
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 w-full space-y-2.5">
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between mb-1">
                    ...
                    {isMet ? (
                      <span className="flex items-center gap-1 text-[10px] font-mono font-bold text-emerald-600 dark:text-emerald-400 uppercase">
                  ...
                  <div className="w-full bg-slate-200 dark:bg-black/40 rounded-full h-1.5 overflow-hidden mb-1">
                    <div
                      className={`h-full rounded-full transition-all duration-500 ${
                        isMet ? 'bg-emerald-500' : 'bg-slate-900 dark:bg-cyan-400'
                      }`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
          ...
          {isAllComplete && (
            <div className="p-3.5 rounded-xl bg-amber-50 dark:bg-amber-500/15 border border-amber-200 dark:border-amber-500/40 flex items-center gap-3 shadow-sm">
              <div className="p-2 rounded-lg bg-amber-100 dark:bg-amber-500/25 text-amber-700 dark:text-amber-300 shrink-0">
